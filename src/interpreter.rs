...
impl Interpreter {
		...
		for i in 0..scope_len {
			if let Some(var) = self.scopes[(scope_len - 1) - i].variables.get_mut(&ident) {
				if discriminant(&var.var_type) != discriminant(&value_type) {
					return Err(format!(
						"Implicit conversion from {:?} to {:?} is not supported",
	...
	pub fn init_var(&mut self, ident: String, value: Value, var_type: Type) -> Result<(), String> {
		self.get_scope().variables.insert(
			ident,
			Variable::new(
				value.clone(),
				match var_type == Type::Auto {
	...
	match expr {
		...
		Expr::Sum { sign, summands } => {
			...
			if sign == Sign::Equal {
				...
				match (left, right) {
					(Value::_String(lstr), Value::_String(rstr)) => match sign {
						Sign::Concat => Ok(Value::_String(lstr + &rstr)),
						_ => Err(format!("Cannot use sign {:?} on Strings.", sign)),
					},
					(Value::_Int(lint), Value::_Int(rint)) => match sign {
					...
					(Value::_Bool(lbool), Value::_Bool(rbool)) => match sign {
		...
		Expr::Ident(ident) => Ok(ctx.get_var(ident)?.value.clone()), //_ => unimplemented!(),
	}
}

fn truthy(value: Value) -> bool {
	match value {
		Value::_String(s) => !s.is_empty(),
		Value::_Int(i) => i != 0,
		Value::_Bool(b) => b,
	}
}
...
pub fn run_code(ctx: &mut Interpreter, nodes: Vec<Node>) -> Result<(), String> {
	...
	while current < nodes.len() {
		match nodes[current].clone() {
			...
			Node::Exit(exitexpr) => {
				...
				match value {
					Value::_Int(exit_code) => {
						...
						ctx.exit_code = Some(exit_code as i32);
						return Ok(());
					}
			...
			Node::If { expr, body } => {
				let value = eval_expr(expr, ctx)?;
				let Node::Scope(body_nodes) = *body else {
					panic!("Expected Node::Scope for if body. This should never happen.");
				};
				if truthy(value) {
